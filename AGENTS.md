...
- Config keys/values in `requirements.txt`, env-var names, URLs, error strings, regex patterns
...
This repository is in early development. The `gptexcel/` package exists but is essentially empty (`__init__.py` only). The `setup.py`, `README.md`, and CI workflow still reference a previous project (`digestvid`). These will need to be updated to reflect the new `gptexcel` project.
...
- `gptexcel/` — Main Python package (currently empty, to be built out)
- `setup.py` — Package configuration (still references `digestvid`; needs updating for `gptexcel`)
- `Govern_partial.json` — NIST CSF governance audit template data (not part of the Python package)

## CI/CD

GitHub Actions workflow (`.github/workflows/publish-to-pypi.yml`) publishes to PyPI on tag push using trusted publishing. It builds with `setup.py sdist bdist_wheel`.

## Dependencies

Core dependencies (from `setup.py`): moviepy, openai-whisper, openai, yt_dlp, pyyaml. These are inherited from the previous `digestvid` project and should be revised when `gptexcel` functionality is defined.

## Key Notes

- Config file (`config.yaml`) is gitignored — used for runtime configuration
